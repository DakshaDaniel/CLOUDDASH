...
tickers_geo = tickers[['ticker', 'name', 'sector', 'country', 'latitude', 'longitude']].dropna()

tickers_geo.drop(['latitude', 'longitude', 'sector'], axis=1, inplace=True)

tickers_geo.index = range(tickers_geo.shape[0])

geo_tickers_exclude = DataTable(
  id = 'geo_tickers_exclude',
  columns = [{'name':col, 'id': col} for col in tickers_geo.columns],
                ...
                html.Div(
                 ...
                 html.Div(   
                  ...
                  dcc.Dropdown(id='Inv_Rev_period', options = options_period, value = 'annual',
                   style={'width':'150px', 'margin':'0 auto', 'color':'blue', 'verticalAlign':'top'}),    
                  html.H3('Organizations to Exclude'),   
                ...
                html.Div(
                 ...
                 html.Div(
                  ...
                  dcc.Dropdown(id='treemap_subsectors_focus', options=subsectors_focus, 
                   value='All', style={'width':'290px', 'margin':'0 auto', 
                                       'color':'blue', 'horizontalAlign':'left'}),],
                  style={'width': '300px', 'height':'300', 'display':'inline-block',
                         'margin':'0px 5px 0px 5px', 'verticalAlign':'top'}),           
                 html.Div(
                  children = [
                  html.H3('Organizations to Exclude'), treemap_tickers_exclude],
#                  dcc.Dropdown(id='treemap_tickers', options=tickers.ticker.unique(), value=[], multi=True)],
                 ...
                 html.Div(
                  children = [
                  html.H3('Period'),
                  dcc.Dropdown(id='treemap_period', options = options_period, value = 'annual',
                ...
                html.Div(
                 ...
                 html.Div(
                  ...
                  style={'width':'250px', 'height':'450px', 'display':'inline-block',
                         ...
                         'background-position':'center', 'background-size':'250px 250px'}),
                 html.Div(
                  children = [dcc.Graph(id='bar_columns')], 
                  style={'width':'900px', 'height':'450px','display':'inline-block'})],
                 style={'width':'1400px', 'height':'450px', 'display':'inline-block',
                        'border':'1px solid black'}),    
                 ...
                 html.Div(
                  ...
                  html.Div(    
                   ...
                   dcc.Dropdown(id='scatter_bar_ticker', 
                       ...
                       style={'width':'350px', 'margin':'0 auto', 'color':'blue', 'verticalAlign':'top'}),    
                   html.Br(),
                   html.Br(),
                   html.H3('Period'),
                   ...
                   html.Br(),
                   html.H3('Radius'),
                   dcc.Dropdown(id='column_r', options = options_columns_pos, value='cash',
                                style={'width':'300px', 'margin':'0 auto',
                                       'color':'blue', 'verticalAlign':'top'})],
                ...
                html.Div(
                 ...
                 html.Div(
                  ...
                  dcc.Graph(id='tcc3_bar',
                            style={'width':'900px', 'height':'800px', 'display':'inline-block'})],
                   style={'width':'900px', 'height':'900px', 'display':'inline-block'})],
                 style={'display':'inline-block', 'vertical-align':'center'}),   
# TCC 3 ()        
                html.Div(
                 children = [
...
# Input(component_id='treemap_sectors_exclude', component_property='selected_rows')
)

def update_geo(ticks):       
    df = df_annual_usd
    
    if ticks: tickers_list = list(tickers_geo.loc[ticks, 'ticker'])
    else: tickers_list = []
    
    df = df[['name', 'ticker', 'longitude', 'latitude', 'totalAssets', 'inventory', 'date2']].dropna()
    ...
    geo_fig = px.scatter_geo(ddf, 
                     lat="latitude", lon="longitude", color="inventory",
                     color_continuous_scale='RdYlGn_r',
                     animation_frame="date2",
                     hover_name="name",
                     hover_data=['ticker', 'name'],        
                     size=np.array(ddf['totalAssets']),
                     size_max=35,        
    ...
    geo_fig.update_layout(geo = dict(
        ...
        oceancolor = 'rgb(151, 255, 255)', subunitcolor = 'rgb(254, 163, 27)', subunitwidth=3))
    geo_fig.update_geos(bgcolor='black', rivercolor='rgb(151, 255, 255)', riverwidth=2)
...
def update_treemap(per, sector, subsector, ticks, country):       
    if per=='annual': df = df_annual_usd
    if per=='quarterly': df = df_quartal_usd
        
    if ticks: tickers_list = list(tickers.loc[ticks, 'ticker'])
    else: tickers_list = []    
    
#    if sectors: sectors_list = list(sectors_table.loc[sectors, 'Sectors'])
#    else: sectors_list = []
    
    ddf = df[['ticker', 'totalAssets', 'inventory', 'sector', 'subsector', 'name', 'date']].dropna()
    ...
    treemap1 = px.treemap(ddddf[~ddddf.ticker.isin(tickers_list)], 
                  ...
                  color='inventory', hover_name='name', color_continuous_scale='RdYlGn_r')
...
def update_bar_columns(per, ticks, cols):
    ...
    df = df[df_annual_usd.columns.difference(['name', 'currency', 'sector', 'subsector', 'date2',
    ...
    #bar_columns.update_traces(color_discrete_map=['rgb(254, 163, 27)']*len(columns_list))
...
def update_ratios1(ticks):
    ...
    ratios1_fig = px.bar(ratios_melt_cols_ticks, x="date", y="value",
                         ...
                         barmode="group", facet_row="ticker")
...
def update_ratios2(ticks):
    ...
    ratios2_fig = px.bar(ratios_melt_cols_ticks, x="date", y="value",
                         color="variable", color_discrete_sequence= px.colors.sequential.Plasma_r,
                         barmode="group", facet_row="ticker")
...
def update_ratios3(ticks):
    ...
    ratios3_fig = px.bar(ratios_melt_cols_ticks, x="date", y="value",
                         color="variable", color_discrete_sequence= px.colors.sequential.Plasma_r,
                         barmode="group", facet_row="ticker")
